...
///
/// This strategy supports having a family of algorithms that implement fetching a feed in different and unique ways (one could
/// fetch just posts, another one could fetch posts and follows, and so on), and making them interchangeable.
protocol FeedStrategy {
    ...
    /// - parameter offset: the offset, offset=10 will return the 10th item onwards, offset=nil will return the first item onwards
    ...
    /// - parameter offset: the offset, offset=10 will return the 10th item onwards, offset=nil will return the first item onwards
    ///
    /// The only use of this function at the moment is for comparing the current displayed items with the potentially new items in the database
    /// so that if the identifier at the top is different it means that there are new items in the database.
