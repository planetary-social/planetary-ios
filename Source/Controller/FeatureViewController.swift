...
class FeatureViewController: UINavigationController {
    ...
    private func setTabBarItem(title: String?, image named: String?) {
        ...
        guard let image = UIImage(named: name)?.withRenderingMode(.alwaysOriginal) else { return }
        guard let selected = UIImage(named: "\(name)-selected")?.withRenderingMode(.alwaysOriginal) else { return }
