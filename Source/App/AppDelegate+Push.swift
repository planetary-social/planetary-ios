...
extension AppDelegate: UNUserNotificationCenterDelegate {
    ...
    func scheduleLocalNotification(_ report: Report) {
        Bots.current.about(queue: .global(qos: .background), identity: report.message.value.author) { (about, error) in
            Log.optional(error)
            CrashReporting.shared.reportIfNeeded(error: error)
            ...
            case .postReplied:
                ...
                if let what = report.message.value.content.post?.text {
    ...
    func didCreateReportHandler(notification: Notification) {
        ...
        Bots.current.blocks(identity: currentIdentity) { [weak self] blockedIdentities, error in
            ...
            reports.forEach { [weak self] report in
                let notifyingIdentity = report.message.value.author
