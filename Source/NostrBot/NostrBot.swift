...
class NostrBot: Bot {
    ...
    // for nostr we're swapping in relays for pubs
    ...
    func registeredAliases(_ identity: Identity?) async throws -> [RoomAlias] {
        throw NostrBotError.notImplemented
    }
    ...
    func createSecret(completion: SecretCompletion) {
        // need to add nostr secrets.
        Thread.assertIsMainThread()
        
        guard let generatedKey = ssbGenKey() else {
            completion(nil, NostrBotError.unexpectedFault("createSecret failed"))
            return
        }
        ...
        completion(nil, NostrBotError.runtimeError("TODO:createSecret"))
    }
    ...
    func publish(post: Post, completion: PublishCompletion) {
        completion("TODO?", NostrBotError.runtimeError("TODO:publish"))
    }
    ...
    func about(completion: @escaping AboutCompletion) {
        ...
        guard let identity = self._identity else {
            completion(nil, NostrBotError.runtimeError("no ID"))
            return
        }
    ...
    func post(from key: MessageIdentifier) throws -> Message {
        throw NostrBotError.runtimeError("not implemented")
    }
    ...
    func replies(message: MessageIdentifier, wantPrivate: Bool, completion: @escaping PaginatedCompletion) {
        completion(StaticDataProxy(), NostrBotError.runtimeError("TODO:replies"))
    }

    func mentions(completion: @escaping PaginatedCompletion) {
        completion(StaticDataProxy(), NostrBotError.runtimeError("TODO:mentions"))
    }
