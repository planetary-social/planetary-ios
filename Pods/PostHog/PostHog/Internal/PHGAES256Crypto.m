...
static NSString *const kRNCryptManagerErrorDomain = @"com.posthog.crypto";
...
@implementation PHGAES256Crypto
...
{
    NSData *iv = [PHGAES256Crypto randomDataOfLength:kAlgorithmIVSize];
    NSData *salt = [PHGAES256Crypto randomDataOfLength:kPBKDFSaltSize];
...
- (NSData *)encrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        PHGLog(@"Unable to encrypt data", error);
        return nil;
    }
    return cipherData;
}
...
- (NSData *)decrypt:(NSData *)data
{
    ...
    if (result == kCCSuccess) {
        ...
        NSError *error = [NSError errorWithDomain:kRNCryptManagerErrorDomain
                                             code:result
                                         userInfo:nil];
        PHGLog(@"Unable to decrypt data", error);
        return nil;
    }
...
+ (NSData *)randomDataOfLength:(size_t)length
{
    ...
    int result = SecRandomCopyBytes(kSecRandomDefault,
                                    ...
                                    data.mutableBytes);
    if (result != kCCSuccess) {
        PHGLog(@"Unable to generate random bytes: %d", result);
    }
                         ...
                         salt:(NSData *)salt
{
    ...
    if (result != kCCSuccess) {
        PHGLog(@"Unable to create AES key for password: %d", result);
    }
